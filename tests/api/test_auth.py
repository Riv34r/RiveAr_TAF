...
Implements AUTH-001 through AUTH-031 from tests/api/scenarios/auth.md.
"""
...
from api.models.auth import TokenResponse, UserResponse
...
def test_current_user_response_matches_schema(auth_client, customer, seed_manifest):
    ...
    with step("The answer validates against UserResponse"):
        assert_status_code(response, 200)
...
def test_register_creates_customer_and_returns_tokens(auth_client):
    with step("Register a new account"):
        email = fake.unique.email()
        response = auth_client.register(email, "StrongPass1", fake.name())

    with step("A usable token pair comes back"):
    ...
    with step("The account is an active CUSTOMER right away"):
...
def test_register_with_duplicate_email_returns_409(auth_client, customer):
    with step("Register under an email that already has an account"):
        response = auth_client.register(customer["email"], "StrongPass1", "Duplicate")

    with step("The email is refused as taken"):
...
def test_register_with_malformed_email_returns_422(auth_client):
    with step("Register with something that is not an email"):
        response = auth_client.register("not-an-email", "StrongPass1", "X")

    with step("The email is rejected"):
...
def test_register_with_invalid_password_returns_422(auth_client, password):
    ...
    with step(f"Register with the password {password!r}"):
        response = auth_client.register(fake.unique.email(), password, fake.name())

    with step("The password is rejected"):
        assert_error(response, 422, "VALIDATION_ERROR")
...
def test_register_missing_required_field_returns_422(auth_client, missing_field):
    with step(f"Register with no {missing_field}"):
        payload = {
            "email": fake.unique.email(),
    ...
    with step("The missing field is rejected"):
        assert_error(response, 422, "VALIDATION_ERROR")
...
def test_login_with_valid_credentials_returns_a_token_pair(
    ...
    with step("Note when the account last logged in"):
    ...
    with step("The login was recorded on the account"):
        current_user = auth_client.get_current_user(token_pair.access_token).json()
...
def test_login_with_wrong_password_returns_401(auth_client, customer):
    ...
    with step("The credentials are refused"):
...
def test_unknown_email_is_indistinguishable_from_wrong_password(
    ...
    with step("Neither answer tells an attacker which account exists"):
...
def test_login_to_a_disabled_account_returns_401(admin_client, factory, auth_client):
    with step("Given a disabled account"):
        new_customer = disabled_customer(admin_client, factory)

    with step("Log in with its own valid credentials"):
        response = auth_client.login(
            new_customer["attributes"]["email"], new_customer["attributes"]["password"]
        )

    with step("The account is refused as disabled"):
...
def test_malformed_login_request_returns_422(api, payload):
    with step(f"Log in with the body {payload}"):
        response = api.post("/auth/login", json=payload)

    with step("The request is rejected"):
        assert_error(response, 422, "VALIDATION_ERROR")
...
def test_refresh_rotates_into_a_new_token_pair(auth_client, customer, seed_manifest):
    ...
    with step("The refresh token is a new one, and the access token works"):
        assert_status_code(response, 200)
...
def test_refresh_token_cannot_be_reused_after_rotation(
    ...
    with step("Refresh again with the token that was already rotated"):
        reused = auth_client.refresh(token_pair["refresh_token"])

    with step("The spent token is revoked"):
        assert_error(reused, 401, "TOKEN_REVOKED")


@pytest.mark.skip(
    reason="No API-only way to produce an expired refresh token - see the "
    "Blocker note on AUTH-014 in tests/api/scenarios/auth.md"
)
...
def test_malformed_refresh_token_returns_401(auth_client):
    with step("Refresh with something that is not a token"):
        response = auth_client.refresh("not.a.token")

    with step("The token is rejected as invalid"):
...
def test_refresh_as_a_since_disabled_user_returns_401(
    ...
    with step("Disable an account that is already logged in"):
        new_customer, token_pair = logged_in_customer
        disable(admin_client, new_customer)

    with step("Refresh with its still-valid refresh token"):
        response = auth_client.refresh(token_pair["refresh_token"])

    with step("The disabled account cannot refresh its way back in"):
        assert_error(response, 401, "USER_DISABLED")
...
def test_logout_revokes_the_refresh_token(auth_client, customer, seed_manifest):
    ...
    with step("The refresh token no longer works"):
        reused = auth_client.refresh(token_pair["refresh_token"])
        assert_error(reused, 401, "TOKEN_REVOKED")
...
def test_logout_is_idempotent_for_an_already_revoked_token(
    ...
    with step("The repeat logout is accepted, not an error"):
        assert_status_code(response, 204)
...
def test_malformed_refresh_token_on_logout_returns_401(auth_client):
    with step("Log out with something that is not a token"):
...
def test_valid_access_token_returns_identity(auth_client, customer, seed_manifest):
    ...
    with step("The identity is that customer's"):
        assert_status_code(response, 200)
...
def test_no_token_returns_401(api):
    with step("Ask who we are without a token"):
...
def test_malformed_access_token_returns_401(auth_client):
    with step("Ask who we are with something that is not a token"):
...
def test_expired_access_token_returns_401(api, auth_client, customer):
    with step("Mint a token that lives for one second, and wait it out"):
        minted = api.post(
            "/test/token", json={"email": customer["email"], "ttl_seconds": 1}
        ).json()
        time.sleep(1.5)

    with step("Ask who we are with the expired token"):
        response = auth_client.get_current_user(minted["access_token"])

    with step("It is refused as expired, not merely invalid"):
...
def test_token_for_a_since_disabled_user_returns_401(
    ...
    with step("Use its still-valid access token"):
        response = auth_client.get_current_user(token_pair["access_token"])

    with step("The token stops working the moment the account does"):
        assert_error(response, 401, "USER_DISABLED")
...
def test_valid_full_name_update_succeeds(auth_client, logged_in_customer):
    ...
    with step("Change the name on the profile"):
        new_name = fake.name()
        response = auth_client.update_profile(token_pair["access_token"], new_name)

    with step("The new name comes back"):
        assert_status_code(response, 200)
...
def test_update_profile_without_authentication_returns_401(api):
    with step("Update the profile without a token"):
...
def test_invalid_full_name_returns_422(auth_client, logged_in_customer, full_name):
    ...
    with step(f"Set the name to one of {len(full_name)} characters"):
        response = auth_client.update_profile(token_pair["access_token"], full_name)

    with step("The name is rejected"):
        assert_error(response, 422, "VALIDATION_ERROR")
...
def test_change_password_with_correct_current_password_succeeds(
    ...
    with step("Change the password, giving the current one"):
        response = auth_client.change_password(
            token_pair["access_token"], current_password, "NewStrongPass1"
        )
        assert_status_code(response, 204)

    with step("The old password no longer works"):
        old_login = auth_client.login(
            new_customer["attributes"]["email"], current_password
        )
        assert_error(old_login, 401, "INVALID_CREDENTIALS")

    with step("The new one does"):
        new_login = auth_client.login(
            new_customer["attributes"]["email"], "NewStrongPass1"
        )
...
def test_change_password_with_wrong_current_password_returns_401(
    ...
    with step("Change the password, giving the wrong current one"):
        response = auth_client.change_password(
            token_pair["access_token"], "WrongCurrent1", "NewStrongPass1"
        )

    with step("The change is refused"):
        assert_error(response, 401, "INVALID_CREDENTIALS")

    with step("The real password still works"):
        old_login = auth_client.login(
            new_customer["attributes"]["email"], new_customer["attributes"]["password"]
        )
...
def test_weak_new_password_returns_422(auth_client, logged_in_customer):
    ...
    with step("Change the password to one that fails the strength rules"):
        response = auth_client.change_password(
            token_pair["access_token"], new_customer["attributes"]["password"], "weak"
        )

    with step("The new password is rejected"):
        assert_error(response, 422, "VALIDATION_ERROR")

    with step("The old password still works"):
        old_login = auth_client.login(
            new_customer["attributes"]["email"], new_customer["attributes"]["password"]
        )
        assert_status_code(old_login, 200)
...
def test_change_password_without_authentication_returns_401(api):
    with step("Change the password without a token"):
        response = api.post(
